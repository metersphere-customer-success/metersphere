    ...
    <div class="ms-base-header" @click="active(data)">
      <slot name="beforeHeaderLeft">
        <div v-if="data.index" class="el-step__icon is-text" :style="{'color': color, 'background-color': backgroundColor}">
          <div class="el-step__icon-inner" :key="commandStore.forceRerenderIndex">{{ data.index }}</div>
        </div>
...
export default {
  ...
  watch: {
    'commandStore.selectStep': function () {
      if (commandStore.selectStep && commandStore.selectStep.resourceId === this.data.resourceId) {
