<template>
  ...
  <div v-loading="loading">
    <el-row style="margin: 5px">
      <el-col :span="1" class="ms-col-one ms-font" v-show="showBatchOptBtn">
        <el-tooltip
    ...
    <vue-easy-tree
      ...
      ref="stepTree"
      :class="hasResult ? 'shortNodePadding' : 'defaultNodePadding'"
    >
...
import { scenarioSort, commandStructSort } from "@/api/api-automation";
...
export default {
  ...
  computed: {
    showBatchOptBtn() {
      if (this.commandViewStruct && this.commandViewStruct.length >= 2) {
        let total = 0;
        if (this.commandViewStruct[0].hashTree) {
          total += this.commandViewStruct[0].hashTree.length;
        }
        if (this.commandViewStruct[1].hashTree) {
          total += this.commandViewStruct[1].hashTree.length;
        }
        if (this.commandViewStruct[2].hashTree) {
          total += this.commandViewStruct[2].hashTree.length;
        }

        return total > 0;
      }
      return this.scenarioDefinition.length > 0;
    },
  },
  data() {
    return {
  ...
  created() {
    ...
    this.$EventBus.$on("handleCustomCommandREFEvent", this.generateREFNodeIds);
  },
  beforeDestroy() {
    this.$EventBus.$off("autoOpenExpansion", this.autoOpenExpansion);
    this.$EventBus.$off("handleCustomCommandREFEvent", this.generateREFNodeIds);
  },
  watch: {
    "commandStore.refreshUiCustom"(value) {
      if (value) {
        this.getUiScenario();
        commandStore.refreshUiCustom = false;
      }
    },
  ...
  methods: {
    /**
     *  自定义指令配置处理 Sence v2.4
     */
    fillMainStepHashTreeData() {
      this.preProcessor();
      let main = this.commandViewStruct.find((item) => {
        return item.debuggerDetail && item.debuggerDetail.type === "MAIN";
      });
      if (main) {
        main.hashTree = this.scenarioDefinition;
      }

      this.currentScenario.commandViewStruct = this.commandViewStruct;

      this.postProcessor();
    },
    preProcessor() {
      this.handleInitStruct();
      this.clearDebuggerResult();
    },
    postProcessor() {
      this.handleParamStruct();
    },
    clearDebuggerResult() {
      this.clearResult(this.commandViewStruct);
    },
    handleParamStruct() {
      //处理入参 出参 信息
      let processor = new ParamProcessor();
      processor.convert(this.commandViewStruct, this.currentScenario.variables);
    },
    handleInitStruct() {
      if (this.currentScenario.commandViewStruct) {
        if (Array.isArray(this.currentScenario.commandViewStruct)) {
          this.commandViewStruct = this.currentScenario.commandViewStruct;
        } else {
          try {
            this.commandViewStruct = JSON.parse(
              this.currentScenario.commandViewStruct
            );
          } catch (e) {
            this.commandViewStruct = [];
          }
        }
        //检测当前hashtree 是否有 resourceId
        if (this.commandViewStruct && this.commandViewStruct.length > 0) {
          this.commandViewStruct.forEach((e) => {
            if (!e.resourceId) {
              e.resourceId = getUUID();
            }
            e.disable = false;

            //除了 Main 其他的都要 改为false
            e.variableEnable = false;
            if (e.debuggerDetail && e.debuggerDetail.type === "MAIN") {
              e.variableEnable = true;
            }
          });
          return;
        }
      ...
      this.commandViewStruct.push(
        this.generateCustomCommandStruct(
          "前置步骤",
          true,
          {
            type: "PRE",
            name: "前置步骤",
            tip: "前置步骤在复制和引用时，不会被导入，仅用于当前指令的调试",
            disableStepOtions: true,
            disableShowDetailPane: true,
          },
          false
        )
      );
      ...
      this.commandViewStruct.push(
        this.generateCustomCommandStruct(
          ...
          true,
          {
            type: "POST",
            name: "后置步骤",
            tip: "后置步骤在复制和引用时，不会被导入，仅用于当前指令的调试",
            disableStepOtions: true,
            disableShowDetailPane: true,
          },
          false
    ...
    generateCustomCommandStruct(
      ...
      let scenario = {
        id: getUUID(),
        resourceId: getUUID(),
        checkDisabled: useDebugger,
        name: name ? name : this.currentScenario.name,
        ...
        browser: this.currentScenario.scenarioDefinition
          ? this.currentScenario.scenarioDefinition.browser
          : "CHROME",
        ...
        projectId: this.currentScenario.projectId
          ...
          : this.projectId,
        headlessEnabled: !this.currentScenario.scenarioDefinition
          ? ""
          : this.currentScenario.scenarioDefinition.headlessEnabled,
        baseURL: this.currentScenario.scenarioDefinition
          ? this.currentScenario.scenarioDefinition.baseURL
          : "",
        enable: true,
        variableEnable: variableEnable,
      };

      // if (debuggerDetail && debuggerDetail.type === "MAIN") {
    ...
    handleNodeRel() {
      ...
      for (let i = 0; i < this.tempStepTreeCache.length; i++) {
        let e = this.tempStepTreeCache[i];
        if (e.useDebugger) {
          continue;
        }
    ...
    generateREFNodeIds() {
      ...
      this.GENARATE_REF_TIMER = setTimeout(() => {
        ...
        this.doGenerateREFNode(this.commandViewStruct);
      }, 100);
    },
    ...
    checkAllowDrop(draggingNode, dropNode, type) {
      ...
      if (
        ...
        if (
          (type === "inner" && dropNode.parent && dropNode.parent.level >= 1) ||
          (dropNode.parent &&
            dropNode.parent.data &&
            dropNode.parent.data.referenced === "REF")
        ) {
          return false;
        } else {
          return true;
      ...
      if (type !== "inner") {
        ...
        dropNode.data.commandType === COMMAND_TYPE_COMBINATION ||
        dropNode.data.useDebugger
      ) {
        return true;
      }
    ...
    enableAll() {
      ...
      this.stepStatus(resourceIds, this.commandViewStruct);
    },
    disableAll() {
        ...
        callback: (action) => {
          if (action === "confirm") {
            this.getAllResourceIds().forEach((item) => {
              for (let i = 0; i < this.commandViewStruct.length; i++) {
                this.recursionDelete(item, this.commandViewStruct[i].hashTree);
              }
            });
    ...
    autoOpenExpansion(resourceIds) {
      ...
      this.changeNodeStatus(resourceIds, this.commandViewStruct);
            ...
            if (action === "confirm") {
              ...
              if (index > 0) {
                ...
                if (hashTree.length > 0) {
                  ...
                  if (
                    this.commandViewStruct &&
                    this.commandViewStruct.length > 1
                  ) {
                    this.nodeClick(this.commandViewStruct[1], node);
                  } else {
                    this.$emit("changeCurrentCommand", {});
                  }
                }
    ...
    handleRunScenario() {},
    sort() {
      if (this.commandViewStruct) {
        commandStructSort(this);
        return;
      }
      scenarioSort(this);
    },
    ...
    getUiScenario() {
      ...
      commandStore.scenarioMap.set(this.currentScenario.id, 0);
        ...
        getUiScenario(this.currentScenario.id).then((data) => {
          if (data) {
            ...
            if (obj) {
              ...
              if (data.commandViewStruct) {
                this.currentScenario.commandViewStruct = data.commandViewStruct;
              }
              //填充到渲染结构的主数据中
              this.fillMainStepHashTreeData();
              ...
              if (obj.hashTree.length) {
                ...
                if (
                  ...
                  commandStore.selectCommand = this.commandViewStruct[1];
                  commandStore.selectStep = this.commandViewStruct[1];
                  this.$emit("changeCurrentCommand", this.commandViewStruct[1]);
                } else {
                  commandStore.selectCommand = this.customCommand.hashTree[0];
            ...
            if (this.currentScenario.copy) {
              ...
              commandStore.scenarioMap.set(this.currentScenario.id, 1);
            }
          } else {
            this.scenarioDefinition = this.value.scenarioDefinition
              ...
              : [];
            this.fillMainStepHashTreeData();
            if (this.commandViewStruct && this.commandViewStruct.length > 1) {
              commandStore.selectCommand = this.commandViewStruct[1];
              commandStore.selectStep = this.commandViewStruct[1];
              this.$emit("changeCurrentCommand", this.commandViewStruct[1]);
            } else {
