<template>
  ...
  >
    ...
    <!--             :style="{ height: '1500px', 'overflow-y': 'auto' }" :page-mode="true" -->
    <div class="el-tree__empty-block" v-if="isEmpty">
      <span class="el-tree__empty-text">{{ emptyText }}</span>
    </div>
    <performance-list
      class="performance-list-wrap scroll-touch"
      :page-mode="pageMode"
...
import PerformanceList from "./virtualScrollList/index";
export default {
  name: "ElTree",
  ...
  created() {
    ...
    this.$on("tree-node-drag-start", (event, treeNode) => {
      if (
        ...
        !this.allowDrag(treeNode.source)
      ) {
      ...
      dragState.draggingNode.node = dragState.draggingNode.source;
      // this.$emit('node-drag-start', treeNode.node, event);
      this.$emit("node-drag-start", treeNode.source, event);
    });

    this.$on("tree-node-drag-over", (event, treeNode) => {
      const dropNode = findNearestComponent(event.target, "ElTreeNode");
      dropNode.node = dropNode.source;
      ...
      if (typeof this.allowDrop === "function") {
        ...
        if (dropNode && dropNode.node.data && dropNode.node.data.referenced === 'REF'){
          userAllowDropInner = false;
          dropInner = false;
        } else {
    ...
    this.$on("tree-node-drag-end", (event) => {
      ...
      if (draggingNode && dropNode) {
        ...
        if (dropType === "before") {
          ...
          if (dropNode && dropNode.node.data && dropNode.node.data.referenced) {
  ...
  mounted() {
    if(!this.elType){
      this.elListener = this.$parent.$parent.$parent.$parent.$el.querySelector(".ms-aside-node-tree");
    }

    if(this.elType > 0){
      this.elListener = this.$parent.$parent.$parent.$parent.$parent.$el.querySelector(".report-container")
    }
    this.initTabIndex();
