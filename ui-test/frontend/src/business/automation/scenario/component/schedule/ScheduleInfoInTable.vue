<template>
    ...
    <span v-if="schedule.scheduleStatus === 'OPEN'">
      <el-tooltip placement="bottom-start" effect="light">
        <div slot="content">
          {{ $t('api_test.home_page.running_task_list.table_coloum.run_rule') }}: {{ schedule.scheduleCorn }}<br/>
...
export default {
    ...
    methods: {
      hasPermissions() {
        return hasPermission('PROJECT_UI_SCENARIO:READ+SCHEDULE');
      },
        scheduleTask() {
            if(!hasPermission('PROJECT_UI_SCENARIO:READ+SCHEDULE')) {
              return;
            }
