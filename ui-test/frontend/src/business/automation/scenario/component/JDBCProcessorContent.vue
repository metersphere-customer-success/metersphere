...
import ApiRequestMethodSelect from "@/business/automation/scenario/component/case/collapse/ApiRequestMethodSelect";
import MsCodeEdit from "metersphere-frontend/src/components/MsCodeEdit";
import MsApiScenarioVariables from "./ApiScenarioVariables";
import {parseEnvironment} from "@/api/EnvironmentModel";
import ApiEnvironmentConfig from "@/business/definition/environment/ApiEnvironmentConfig";
...
export default {
  ...
  watch: {
    ...
    // 接口/用例 右上角公共环境监听
    'commandStore.useEnvironment': function () {
      if (!this.scenarioId) {
        this.getEnvironments(commandStore.useEnvironment);
      }
    },
    // 接口/用例 自身环境监听
  ...
  methods: {
    ...
    itselfEnvironment(environmentId) {
      ...
      this.result = this.$get('/api/environment/list/' + id).then(response => {
    ...
    getEnvironments(environmentId) {
      ...
      if (commandStore.scenarioEnvMap && commandStore.scenarioEnvMap instanceof Map
        && commandStore.scenarioEnvMap.has(scenarioEnvId)) {
        envId = commandStore.scenarioEnvMap.get(scenarioEnvId);
      }
