...
import {PLUGIN_ELEMENTS} from "@/api/Setting";
import MsJmxStep from "@/business/definition/step/JmxStep";
import {stepCompute, hisDataProcessing} from "@/business/automation/scenario/common/function/api-definition";
import MsPluginContentAssertions from "./PluginContentAssertions";
...
export default {
  ...
  methods: {
    ...
    run() {
      ...
      if (commandStore.scenarioEnvMap && commandStore.scenarioEnvMap instanceof Map
        && commandStore.scenarioEnvMap.has((this.currentScenario.id + "_" + this.request.projectId))) {
        currentEnvironmentId = commandStore.scenarioEnvMap.get((this.currentScenario.id + "_" + this.request.projectId));
      }
      let debugData = {
    ...
    getPlugin() {
      ...
      if (id) {
        this.$get('/plugin/get/' + id).then(response => {
