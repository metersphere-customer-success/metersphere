...
import MsApiAuthConfig from "@/business/automation/scenario/edit/command/auth/ApiAuthConfig";
import {REQUEST_HEADERS} from "@/api/constants";
import {TYPE_TO_C} from "@/api/Setting";
import {KeyValue} from "@/api/ApiTestModel";
import {isProjectVersionEnable} from "metersphere-frontend/src/api/version"
import {getCurrentProjectID} from "metersphere-frontend/src/utils/token";
import {hasLicense} from "metersphere-frontend/src/utils/permission";
import {getProjectVersions} from "@/business/automation/ui-automation";
    ...
    getVersionOptions() {
      if (hasLicense()) {
        getProjectVersions(getCurrentProjectID()).then(response =>{
          this.versionOptions = response.data.data.filter(v => v.status === 'open');
    ...
    checkVersionEnable() {
      ...
      if (hasLicense()) {
        isProjectVersionEnable(this.projectId).then(response =>{
          this.projectVersionEnable = response.data.data;
        })
      }
    }
  }
