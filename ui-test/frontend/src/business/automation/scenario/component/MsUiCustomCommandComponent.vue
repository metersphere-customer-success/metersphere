<template>
  ...
  >
    <template v-slot:beforeHeaderLeft>
      <div class="step-content" v-if="!useDebugger">
        <div
          v-if="scenario.index"
        ...
        <span
          style="
            ...
            line-height: 20px;
          "
          v-if="isCurrentReference && !editNameStart && !editable"
        >
        ...
        <span
          ...
          "
          v-if="
            isCopy && (!checkStepDisabledForREF || !editable) && !editNameStart
          "
        >
        ...
        <span style="width: 5px; display: inline-block"
          ><i
            ...
            @click="editName"
            v-show="checkStepDisabledForREF && editable && !editNameStart && !isReference"
        /></span>
      ...
      <div v-else>
        <span
          ...
          >{{ debuggerDetail.name }}</span
        >
        <!-- 检测是否存在提示信息 -->
        <el-tooltip class="item" effect="dark" placement="right" v-if="debuggerDetail.tip">
          <div slot="content">
            {{ debuggerDetail.tip }}
          </div>
          <i
            :style="{ 'font-size': 10 + 'px', 'margin-left': 5 + 'px' }"
            class="el-icon-info"
          ></i>
        </el-tooltip>
      </div>
...
export default {
  ...
  computed: {
    useDebugger() {
      return this.scenario ? this.scenario.useDebugger : false;
    },
    debuggerDetail() {
      return this.scenario && this.scenario.debuggerDetail
        ? this.scenario.debuggerDetail
        : {};
    },
    isDeletedOrRef() {
      if (
    ...
    title() {
      ...
      let scType = isCustomCommand
        ? this.$t("ui.custom_command_title")
    ...
    isReference() {
      return !this.scenario.pREF && this.scenario.referenced === "REF";
    },
    isCopy() {
