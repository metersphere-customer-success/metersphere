...
import AllApiDefinition from "@/api/all-api-definition";
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      ...
      let url = "/ui/scenario/report/update";
    ...
    initMessageSocket() {
      let url = "/websocket/" + this.reportId;
      this.messageWebSocket = socket(url);
      ...
      this.messageWebSocket.onclose = this.onClose;
      this.messageWebSocket.onerror = this.cleanHeartBeat;
    },
    getReport() {
      let url = "/ui/scenario/report/get/" + this.reportId + "?selectReportContent=true";
      // let url = "/ui/scenario/report/get/" + this.reportId + "?selectReportContent=true";
      AllApiDefinition.getReport(url).then(response => {
    ...
    onClose(){
      if(!this.hasResult){
        console.log("ws has been closed , get result~");
        this.getReport();
    ...
    onMessage(e) {
      ...
      if (e.data && e.data.startsWith("result_")) {
        try{
          ...
          new ParamProcessor()
            ...
            .fillRunTimeData(this.scenario.scenarioDefinition, results);
        }catch(e){
          //
        }
      }
      if (e.data && e.data.indexOf("MS_TEST_END") !== -1) {
        ...
        try{
          ...
          //websocket 已经关闭
          console.log("ws has already closed !");
        }
        this.$EventBus.$emit('hide', this.scenarioId);
        this.$emit('refresh', this.debugResult);
        this.hasResult = true;
