<template>
    <ms-container v-loading="loading" :element-loading-text="$route.params.view ? $t('ui.executing') : $t('ui.loading')"
                  class="loading-vid">
            ...
            <el-card class="report-body">
                <section class="report-container" v-if="this.report.testId">
                    ...
                    <ms-api-report-view-header
                            :show-cancel-button="finalShowCancelButton"
                            :show-rerun-button="showRerunButton"
                            :show-report-name-button="showReportNameButton"
                            :is-plan="isPlan"
...
import {RequestFactory} from "@/api/ApiTestModel";
...
import MsUiEnvConfig from "@/business/automation/report/components/UiEnvConfig";
...
export default {
    ...
    data() {
        return {
            ...
            tempResult: [],
            projectEnvMap: {},
            finalShowCancelButton: false,
            showReportNameButton: true,
        }
    },
    activated() {
        this.descFlag = false;
            ...
            if (data) {
                ...
                if (this.report.reportVersion && this.report.reportVersion > 1) {
                    ...
                    if ('RUNNING' === data.status && !this.descFlag) {
                        setTimeout(this.doGetReport, 2000)
                    } else {
                        if (data.content) {
                ...
                let envConfig = JSON.parse(data.envConfig);
    ...
    mounted() {
        if (this.$route.params && Object.hasOwn(this.$route.params, 'reportId') && !this.reportId) {
            this.reportId = this.$route.params.reportId;
        }
        this.getReport();
        this.$EventBus.$on('projectChange', this.handleProjectChange);
        //路由优先决定是否隐藏取消按钮
        if (this.$route.params && Object.hasOwn(this.$route.params, 'showCancelButton')) {
            this.finalShowCancelButton = this.$route.params.showCancelButton;
        } else {
            this.finalShowCancelButton = this.showCancelButton;
        }
        if (this.$route.params && !this.$route.params.showReportNameButton) {
            this.showReportNameButton = false;
        }
    },
    destroyed() {
        this.descFlag = true;
    ...
    computed: {
        ...
        isNotRunning() {
            return "RUNNING" !== this.report.status && "PENDING" !== this.report.status;
        },
        projectId() {
