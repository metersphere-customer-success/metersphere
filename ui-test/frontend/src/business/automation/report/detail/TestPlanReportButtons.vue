...
import TestPlanApiReport from "@/business/automation/report/detail/TestPlanApiReport";
import {generateShareInfoWithExpired} from "@/business/network/share";
import TestPlanReportEdit from "./TestPlanReportEdit";
import {editPlanReport, saveTestPlanReport} from "@/business/network/test-plan";
import {CURRENT_LANGUAGE} from "@/i18n/lang/i18n";
...
export default {
  ...
  methods: {
    ...
    getProjectApplication() {
      this.$get('/project_application/get/' + getCurrentProjectID() + "/TRACK_SHARE_REPORT_TIME").then(res => {
        if (res.data) {
