          ...
          <el-tab-pane :label="$t('schedule.task_config')" name="first">
            ...
            <div style="padding-top: 10px">
              ...
              <env-popover
                  ...
                  :show-config-button-with-out-permission="true"
...
import {createSchedule, getScheduleByIdAndType} from "@/business/automation/ui-automation"
import {uiScenarioEnv} from "@/api/scenario";
import {getProjectMembers} from "metersphere-frontend/src/api/version";
...
export default {
  ...
  methods: {
    ...
    setProjectEnvMap(projectEnvMap) {
      this.projectEnvListMap = projectEnvMap;
      this.runConfig.envMap = strMapToObj(projectEnvMap);
    },
    showPopover() {
      this.projectIds.clear();
      uiScenarioEnv(this.request).then((res) => {
        let data = res.data;
        if (data) {
          for (let d in data) {
    ...
    getResourcePools() {
      this.result = this.$get('/testresourcepool/list/quota/valid').then(response => {
    ...
    updateTask(param) {
      this.result = this.$post('/api/schedule/updateEnableByPrimyKey', param, response => {
        ...
        if (this.paramRow.redirectFrom === 'testPlan') {
          ...
          paramTestId = this.paramRow.id;
          this.scheduleTaskType = "UI_SCENARIO_TEST";
        }
    ...
    initUserList() {
      this.result = getProjectMembers().then(response => {
    ...
    saveSchedule() {
      ...
      let url = '/ui/automation/schedule/create';
      if (this.scheduleTaskType === "TEST_PLAN_TEST") {
        ...
        if (param.id) {
          url = '/ui/automation/schedule/update';
        }
      }
      createSchedule(url, param).then(() => {
