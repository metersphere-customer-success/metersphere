...
<!--          <ms-table-column
      ...
      <div>
        ...
        <el-drawer
          ...
          :modal="false"
          size="90%"
          :before-close="resetRunBtn"
        >
          <sysn-api-report-detail
            v-loading="reportLoading"
            @finish="reportLoading = false"
            @refresh="search"
            :debug="true"
...
export default {
  ...
  components: {
    ...
    MsTableMoreBtn: () => import("../scenario/TableMoreBtn"),
  ...
  data() {
    return {
      ...
      fields: getCustomTableHeaderByDefault("UI_CUSTOM_COMMAND"),
  ...
  methods: {
    ...
    getProjectName() {
      this.$get("project/get/" + this.projectId).then((response) => {
    ...
    search(projectId) {
      this.$EventBus.$emit("caseCommandConditionBus", this.condition)
    ...
    moveSave(param) {
      ...
      if (!this.isMoveBatch) url = "/ui/automation/batch/copy";
    ...
    batchEdit(form) {
      ...
      if (form.type === "projectEnv") {
        ...
        this.$post("/ui/automation/batch/update/env", param).then(() => {
          this.$success(this.$t("commons.save_success"));
          this.search();
        });
      } else {
        ...
        this.buildBatchParam(param);
        param.scenarioType = this.currentType;
        this.$post("/ui/automation/batch/edit", param).then(() => {
          this.$success(this.$t("commons.save_success"));
          this.search();
        });
      }
      ...
      return new Promise((resolve) => {
        getUiAutomation(scenarioId).then(response => {
            if (response.data) {
              ...
              this.currentScenario.clazzName = TYPE_TO_C.get("customCommand");
    ...
    exportSide() {
      ...
      if (param.ids === undefined || param.ids.length < 1) {
        this.$warning(this.$t("ui.check_command"));
        return;
      }
      // this.result.loading = true;
      this.projectId = getCurrentProjectID();
