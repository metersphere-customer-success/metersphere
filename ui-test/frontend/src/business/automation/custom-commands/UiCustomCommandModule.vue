<template>
  <div>
    ...
    <ui-import ref="apiImport" :moduleOptions="data" @refreshAll="$emit('refreshAll')" scenarioType="customCommand"/>
  </div>
...
import {
  ...
  dragScenarioModule,
  editScenarioModule,
  getScenarioModulesOpt,
  posScenarioModule
} from "@/business/network/ui-scenario-module";
import UiImport from "../scenario/UiScenarioImport";
...
export default {
  ...
  created(){
    this.$EventBus.$on("caseCommandConditionBus", (param)=>{
      this.param = param;
    })

  },
  beforeDestroy() {
    this.$EventBus.$off("caseCommandConditionBus", (param)=>{
      this.param = param;
    })
  },
  methods: {
    ...
    list(projectId) {
      if (this.isPlanModel) {
        ...
        getScenarioModulesOpt(projectId ? projectId : this.projectId, this.isTrashData, this.param, this.currentType).then((data) => {
          if (data) {
            this.data = data.data;
            this.data.forEach(node => {
              buildTree(node, {path: ''});
