    ...
    <el-row :gutter="5">
      ...
      <el-col :span="15">
        ...
        <div class="scenario-container">
          ...
          <div class="env-bar" v-if="scenarioType==='scenario'">
            <env-popover
...
import {getReportMessageSocket, savePreciseEnvProjectIds, saveScenario} from "@/api/api-automation";
import {
  getOwnerProjects,
  getUiRunDebugData,
  getUiScenarioEnv,
...
import { filter, cloneDeep, findIndex, forEach, map } from "lodash-es";
...
export default {
  ...
  watch: {
    ...
    currentScenario: {
      handler(val) {
        let commandViewStruct = this.currentScenario.commandViewStruct;
        if (
            commandViewStruct && commandViewStruct.length > 0 &&
            (
                commandViewStruct[0].hashTree && commandViewStruct[0].hashTree.length
                ||
                commandViewStruct[1].hashTree && commandViewStruct[1].hashTree.length
                ||
                commandViewStruct[2].hashTree && commandViewStruct[2].hashTree.length
            )
        ) {
          this.readOnlyEnvPopover = false;
        } else {
          this.readOnlyEnvPopover = true;
        }
      },
      immediate: true,
      deep: true
  ...
  mounted() {
    ...
    var that = this;
    this.$EventBus.$off("saveVariables")
    this.$EventBus.$on("saveVariables",function(saveVariables, headers){
      that.saveVariables(saveVariables, headers)
    })
  },
  methods: {
    clickCard() {
    ...
    preSave() {
      ...
      this.$nextTick(() => {
        ...
        this.$emit("save", null, "customCommand");
      });
    },
    async getCustomUiDebugModeName() {
      let userRunMode = localStorage.getItem(
          "CUSTOM_COMMAND_RUN_MODE_" + this.workspaceId
      );
      if (!userRunMode) {
    ...
    async getCustomUiDebugMode() {
      let res = await this.$get(
          "/project_application/get/config/" + this.projectId,
          null
      );
      let projectData = res.data.data;
      if (!projectData || !projectData.uiQuickMenu) {
        return {mode: "server", report: false};
      } else {
        return {mode: projectData.uiQuickMenu, report: false};
      }
    },
    getVariableSize() {
      let size = 0;
    ...
    saveVariables(saveVariables, headers) {
    ...
    onDebugMessage(e) {
      if (e.data && e.data.startsWith("result_")) {
        ...
        this.runningEvaluation(this.mergeDebugHashtree(), resultMap);

        //运行出参结果回显
        new ParamProcessor()
            .getParamProcessor("OUTPUT")
            .fillRunTimeData(this.currentScenario.commandViewStruct, results);
        //发送出参处理事件
      }
    ...
    //TODO merge options replace all data
    mergeDebugHashtree() {
      //this.currentScenario.commandViewStruct ? this.currentScenario.commandViewStruct : this.currentScenario.scenarioDefinition.hashTree
      return this.currentScenario.commandViewStruct
          ? this.currentScenario.commandViewStruct
          : this.currentScenario.scenarioDefinition.hashTree;
    },
    getValidateDebugResult(node, debugResults) {
      ...
      validateCmds = validateCmds.filter((v) => {
        return v.enable;
      });
    ...
    async runDebug(data) {
      ...
      let hashTree = this.mergeDebugHashtree() || [];
      ...
      //补充调试数据
      if (this.tempCurrentScenario.commandViewStruct) {
        this.tempCurrentScenario.scenarioDefinition.hashTree =
            this.tempCurrentScenario.commandViewStruct;
      }
      ...
      await this.getEnv(JSON.stringify(this.tempCurrentScenario.scenarioDefinition));
      this.reportId = getUUID().substring(0, 8);
    },
    handleParamData(scenarioDefinition, variables) {
      let processor = new ParamProcessor();
      processor.convert(scenarioDefinition, variables);
    },
    ...
    fresh() {
      // commandStore.setRefreshUiCustom = true;
      commandStore.refreshUiCustom = true;
    },
    resetDebugBtn() {
      this.debugLoading = false;
      this.debugVisible = false;
    ...
    async validateScenario(hashTree) {
      let validator = this.prepareValidate();
      let data = await validator.validate(true);
      if (data) {
        ...
        getUiScenarioEnv({definition: definition}).then((res) => {
          if (res.data) {
            ...
            if (this.currentScenario && this.currentScenario.env) {
              let temSet = new Set(Object.keys(JSON.parse(this.currentScenario.env)));
