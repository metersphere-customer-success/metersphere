...
import {
  ...
  downloadFileZip,
  downloadMetaData,
  getAllTypeFileMeta, getFileMetaPages
} from "@/api/file";
...
export default {
  ...
  methods: {
    ...
    getProjectFiles() {
      this.data = getFileMetaPages(this.projectId, this.currentPage, this.pageSize, this.condition).then(res => {
        let data = res.data;
    ...
    handleDeleteBatch() {
      operationConfirm(this.$t('project.file_delete_tip', [this.$refs.table.selectIds.length + " 条 "]), () => {
        batchDeleteMetaData(this.$refs.table.selectIds).then(res => {
    ...
    handleDelete(row) {
      operationConfirm(this.$t('project.file_delete_tip', [row.name]), () => {
        deleteFileMetaById( row.id).then(res => {
          this.$success(this.$t('commons.delete_success'));
          this.getProjectFiles();
        });
      });
