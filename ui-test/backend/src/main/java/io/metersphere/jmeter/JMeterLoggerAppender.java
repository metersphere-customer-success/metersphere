    ...
    public void append(ILoggingEvent event) {
        try {
            if (!event.getLevel().levelStr.equals(LogUtil.DEBUG) && StringUtils.isNotEmpty(event.getThreadName())) {
                ...
                String threadName = StringUtils.substringBefore(event.getThreadName(), THREAD_SPLIT);
                ...
                if (!message.toString().contains("java.net.UnknownHostException") && FixedCapacityUtils.containsKey(threadName)) {
