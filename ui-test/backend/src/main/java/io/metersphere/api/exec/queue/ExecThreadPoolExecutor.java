...
import io.metersphere.jmeter.UILocalRunner;
...
public class ExecThreadPoolExecutor {
    ...
    public void addTask(UiJmeterRunRequestDTO requestDTO) {
        ...
        public void run() {
            ...
            if (CollectionUtils.isNotEmpty(msRejectedExecutionHandler.getBufferQueue())) {
                ...
                if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) {
                    UiJmeterRunRequestDTO requestDTO = msRejectedExecutionHandler.getBufferQueue().poll();
                    ExecTask task = new ExecTask(requestDTO);
                    threadPool.execute(task);
    ...
    public void setCorePoolSize(int maximumPoolSize) {
        try {
            // 最多能有500个线程
            maximumPoolSize = Math.min(maximumPoolSize, 500);
    ...
    public void removeQueue(String reportId) {
        // 处理运行中的任务，取消掉
        UILocalRunner.stop(reportId);
        // 检查缓冲区
        Queue<UiJmeterRunRequestDTO> bufferQueue = msRejectedExecutionHandler.getBufferQueue();
