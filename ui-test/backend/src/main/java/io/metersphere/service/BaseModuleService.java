...
import io.metersphere.dto.EditModuleDateDTO;
...
public class BaseModuleService extends NodeTreeService<ModuleNodeDTO> {
    ...
    public ModuleNode getDefaultNodeWithType(String projectId, String type, String defaultName) {
        ...
        if (moduleNode.getLevel() == 1 && moduleNode.getPos() > 1) {
            moduleNode.setPos(1.0);
            extModuleNodeMapper.updateByPrimaryKey(tableName, moduleNode);
        }
        return moduleNode;
    }
    ...
    public List<ModuleNodeDTO> getNodeTreeByProjectIdWithCountOpt(String projectId, Function<QueryNodeRequest, List<Map<String, Object>>> getModuleCountFunc, String defaultName, String keyWord) {
        // 判断当前项目下是否有默认模块，没有添加默认模块
        this.getDefaultNode(projectId, defaultName);
        ...
        if (getModuleCountFunc != null) {
            ...
            buildNodeCount(projectId, moduleNodes, getModuleCountFunc, request);
        }
        return getNodeTrees(moduleNodes);
    }
