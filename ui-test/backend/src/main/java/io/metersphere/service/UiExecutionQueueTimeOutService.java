...
@Service
public class UiExecutionQueueTimeOutService {
    @Resource
    ...
    public void defendQueue() {
        ...
        final long timeout = System.currentTimeMillis() - (2 * 60 * MINUTE_MILLIS);
        // 查询timeout报告
        UiScenarioReportExample scenarioReportExample = new UiScenarioReportExample();
        scenarioReportExample.createCriteria().andStatusEqualTo(ReportStatus.TIMEOUT.name()).andCreateTimeGreaterThanOrEqualTo(timeout);
        List<UiScenarioReportWithBLOBs> uiScenarioReports = uiScenarioReportMapper.selectByExampleWithBLOBs(scenarioReportExample);
        if(CollectionUtils.isEmpty(uiScenarioReports)){
            return;
        }
        addqueue(uiScenarioReports);
        ApiExecutionQueueDetailExample example = new ApiExecutionQueueDetailExample();
        example.createCriteria().andTypeNotEqualTo("loadTest").andSortEqualTo(100);
        ...
        for (ApiExecutionQueueDetail item : queueDetails) {
            ...
            if (report != null && StringUtils.equalsAnyIgnoreCase(report.getStatus(), ReportStatus.TIMEOUT.name())) {
    ...
    private void addqueue(List<UiScenarioReportWithBLOBs> uiScenarioReports){
        uiScenarioReports.forEach(uiScenarioReport->{
            String reportId = uiScenarioReport.getId();
            String projectId = uiScenarioReport.getProjectId();
            String scenarioId = uiScenarioReport.getScenarioId();
            ApiExecutionQueue apiExecutionQueue = new ApiExecutionQueue();
            String queueId = UUID.randomUUID().toString();
            long time = System.currentTimeMillis();
            apiExecutionQueue.setId(queueId);
            apiExecutionQueue.setReportId(reportId);
            apiExecutionQueue.setCreateTime(time);
            apiExecutionQueue.setReportType("iddReport");
            apiExecutionQueue.setRunMode("UI_SCENARIO");
            baseExecutionQueueMapper.insertSelective(apiExecutionQueue);
            ApiExecutionQueueDetail apiExecutionQueueDetail = new ApiExecutionQueueDetail();
            apiExecutionQueueDetail.setId(UUID.randomUUID().toString());
            apiExecutionQueueDetail.setSort(100);
            apiExecutionQueueDetail.setQueueId(queueId);
            apiExecutionQueueDetail.setEvnMap(uiScenarioReport.getEnvConfig());
            apiExecutionQueueDetail.setType(RunModeConstants.SERIAL.toString());
            List projectIds = new ArrayList();
            projectIds.add(projectId);
            apiExecutionQueueDetail.setProjectIds(JSON.toJSONString(projectIds));
            apiExecutionQueueDetail.setRetryEnable(Boolean.FALSE);
            apiExecutionQueueDetail.setReportId(reportId);
            apiExecutionQueueDetail.setTestId(scenarioId);
            apiExecutionQueueDetail.setRetryNumber(0L);
            apiExecutionQueueDetail.setCreateTime(time);
            baseExecutionQueueDetailMapper.insertSelective(apiExecutionQueueDetail);
        });
    }
