...
public class RequestResultExpandDTO extends RequestResult {
    ...
    public RequestResultExpandDTO(UiScenarioReportResultWithBLOBs requestResult) {
        if (requestResult.getBaseInfo() != null) {
            try {
                UiScenarioReportBaseInfoDTO dto = JSON.parseObject(requestResult.getBaseInfo(), UiScenarioReportBaseInfoDTO.class);
