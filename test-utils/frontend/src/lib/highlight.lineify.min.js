var LineifyPlugin = (function () {
  ...
  class LineifyPlugin {
    ...
    _lineify(element) {
      ...
      this.postProcessLineElements(
        element,
        lineElements,
        this.defaultPostProcessingOptions
      );
    ...
    postProcessLineElements(element, lineElements, postProcessingOptions) {
      let lineNumberFormat =
        element.getAttribute("lineify-line-number-format") ||
        postProcessingOptions.lineNumberFormat;
      if (lineNumberFormat) {
        this.insertLineNumSpans(lineElements, lineNumberFormat);
      }
      let linesClassAttributeFormat =
        element.getAttribute("lineify-lines-class-attribute-format") ||
        postProcessingOptions.linesClassAttributeFormat;
    ...
    scan(formatString, s) {
      ...
      if (
        pos >= formatString.length - formatDir.length ||
        !s.endsWith(suffix)
      ) {
        return undefined;
      }
    ...
    insertLineNumSpans(lineElements, lineNumberFormat) {
      for (let i = 0; i < lineElements.length; i++) {
        ...
        lineElement.insertBefore(
          this.createLineNumSpan(lineNum),
      ...
      while (children.length > 0) {
        ...
        if (child.nodeType === Node.ELEMENT_NODE) {
          ...
          if (pos < 0) {
            ...
            let childCopy = document.createTextNode(
              child.data.substring(0, pos + 1)
            );
            this._appendChildCopyHelper(childCopy, elementStack, lineNodes);
            ...
            if (pos + 1 < child.data.length) {
              children.unshift(
                document.createTextNode(child.data.substring(pos + 1))
              );
            }
          }
        }
